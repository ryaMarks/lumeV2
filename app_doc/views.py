...
def orderService(request, nome):  # recebe a solicitacao html e o nome do usuario
    if request.user.is_authenticated:  # se o usuario estiver autenticado (logado)
        ...
        html_string = render_to_string('orderService.html', context=context)  # prepara o template html
        html = HTML(string=html_string)
        pdf = html.write_pdf()  # transforma html em pdf
        response = HttpResponse(pdf, content_type='application/pdf')
        response['Content-Disposition'] = 'filename="OS - ' + lista.nome + '.pdf"'
        return response
    else:  # se o usuario nao estiver autenticado (logado)
        return redirect("/login/")  # manda para a pagina de login
...
def procuracao(request, nome):  # recebe a solicitacao html e o nome do usuario
    if request.user.is_authenticated:  # se o usuario estiver autenticado (logado)
        ...
        pdf = html.write_pdf()  # transforma html em pdf
        response = HttpResponse(pdf,content_type='application/pdf')
        response['Content-Disposition'] = 'filename="PROCURAÇÃO - '+ lista.nome + '.pdf"'
        return response
    else:  # se o usuario nao estiver autenticado (logado)
        return redirect("/login/")   # manda para a pagina de login


def clientTrello(request):
    if request.user.is_authenticated:  # se o usuario estiver autenticado (logado)
        form = ClienteForm()  # carrega o formulario que esta cadastrado em ClientForm
        cliente = getClientData('SALIM JORGE WAQUIM NETO')
        print(cliente)
        return redirect('')
    else:  # se o usuario nao estiver autenticado (logado)
        return redirect("/login/")  # manda para a pagina de login